    ...
    def run(self, num_episodes=1):
        ...
        try:
            for episode in range(num_episodes):
                ...
                summary = 'Episode {:3d} / {:3d} | Timesteps {:4d} | Fruits {:2d}, {:2d} | Rewards {:2d}, {:2d}'
                print(summary.format(episode + 1, num_episodes, self.env.stats.timesteps_survived, self.env.stats.fruits_eaten_1, self.env.stats.fruits_eaten_2, self.env.stats.sum_episode_rewards_1, self.env.stats.sum_episode_rewards_2))
        ...
        while running:
            ...
            if timestep_timed_out or human_made_move:
                ...
                if not is_human_agent:
                    #action = self.agent.act(timestep_result.observation, timestep_result.reward_1, alive_1, alive_2)
                    action = self.agent.act(timestep_result.observation, timestep_result.reward_1)
                self.env.choose_action(action)
                timestep_result = self.env.timestep()
                alive_1 = timestep_result.alive_1
                alive_2 = timestep_result.alive_2
...
class Colors:
    ...
    CELL_TYPE = {
        ...
        CellType.SNAKE_HEAD_2: (139, 0, 0),
        CellType.SNAKE_BODY_2: (255, 0, 0),
