...
class Field(object):
    ...
    def __init__(self, level_map=None):
        ...
        self._level_map_to_cell_type = {
            ...
            'T': CellType.SNAKE_HEAD_2,
            't': CellType.SNAKE_BODY_2,
            '#': CellType.WALL,
            'O': CellType.FRUIT,
            '.': CellType.EMPTY,
        }
    ...
    def find_snake_head_2(self):
        """ Find the snake's head on the field. """
        for y in range(self.size):
            for x in range(self.size):
                if self[(x, y)] == CellType.SNAKE_HEAD_2:
                    return Point(x, y)
        raise ValueError('Initial snake position not specified on the level map')
    ...
    def place_snake_2(self, snake):
        """ Put the snake on the field and fill the cells with its body. """
        self[snake.head] = CellType.SNAKE_HEAD_2
        for snake_cell in itertools.islice(snake.body, 1, len(snake.body)):
            self[snake_cell] = CellType.SNAKE_BODY_2
    ...
    def update_snake_footprint_2(self, old_head, old_tail, new_head):
        """
        Update field cells according to the new snake position.
        ...
        Args:
            ...
            new_head: position of the head after the move.
        """
        self[old_head] = CellType.SNAKE_BODY_2

        # If we've grown at this step, the tail cell shouldn't move.
        if old_tail:
            self[old_tail] = CellType.EMPTY

        # Support the case when we're chasing own tail.z
        if self[new_head] not in (CellType.WALL, CellType.SNAKE_BODY_2) or new_head == old_tail:
            self[new_head] = CellType.SNAKE_HEAD_2
