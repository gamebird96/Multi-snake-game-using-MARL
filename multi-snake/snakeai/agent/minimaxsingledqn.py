...
class MinimaxSingleDeepQNetworkAgent(AgentBase):
    ...
    def __init__(self, model, num_last_frames=4, memory_size=1000):
        ...
        Args:
            model: a compiled DQN model for snake 1.
        ...
        self.memory = ExperienceReplay((num_last_frames,) + model.input_shape[-2:], model.output_shape[-1]//3, memory_size)
        self.frames = None
    ...
    def train(self, env, num_episodes=1000, batch_size=50, discount_factor=0.9, checkpoint_freq=None,
        ...
        for episode in range(num_episodes):
            ...
            while not game_over:
                if np.random.random() < exploration_rate:
                    ...
                    q = q.reshape((env.num_actions, env.num_actions))
                    if alive_1 and alive_2:
                        action = (np.argmax(np.min(q, axis=1)), np.argmax(np.min(q, axis=0)))
                    elif alive_1:
                        #action = (np.argmax(np.min(q1, axis=1)), np.argmin(np.max(q1, axis=0)))
                        action = (np.argmax(np.min(q, axis=1)), np.random.randint(env.num_actions))
                    elif alive_2:
                        #action = (np.argmin(np.max(q2, axis=0)), np.argmax(np.min(q2, axis=1)))
                        action = (np.random.randint(env.num_actions), np.argmax(np.min(q, axis=0)))
                ...
                self.memory.multi_remember(*experience_item_1)
                state = state_next

                # Sample a random batch from experience.

                if alive_1 or alive_2:
                    batch = self.memory.get_multi_batch(
                        model=self.model,
                    ...
                    if batch:
                        ...
                        loss_1 += float(self.model.train_on_batch(inputs, targets))
                    # Sample a random batch from experience.
            ...
            if checkpoint_freq and (episode % checkpoint_freq) == 0:
                self.model.save(f'dqn-s-{episode:08d}.model')
            ...
            print(summary.format(
                episode + 1, num_episodes, loss_1, exploration_rate,
        ...
        self.model.save('dqn-s-final.model')
    ...
    def act(self, observation, reward,  alive_1=True, alive_2=True):
        ...
        q = self.model.predict(state).reshape(3, 3)

        if alive_1 and alive_2: return (np.argmax(np.min(q, axis=1)), np.argmax(np.min(q, axis=0)))
        elif alive_1: return (np.argmax(np.min(q, axis=1)), np.random.randint(3))
        elif alive_2: return (np.random.randint(3), np.argmax(np.min(q, axis=0)))
