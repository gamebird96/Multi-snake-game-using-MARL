...
class MultiDeepQNetworkAgent(AgentBase):
    ...
    def __init__(self, model_1, model_2, num_last_frames=4, memory_size=1000):
        ...
        self.memory_1 = ExperienceReplay((num_last_frames,) + model_1.input_shape[-2:], model_1.output_shape[-1], memory_size)
        self.memory_2 = ExperienceReplay((num_last_frames,) + model_2.input_shape[-2:], model_2.output_shape[-1], memory_size)
        self.frames = None
    ...
    def train(self, env, num_episodes=1000, batch_size=50, discount_factor=0.9, checkpoint_freq=None,
        ...
        for episode in range(num_episodes):
            ...
            while not game_over:
                if np.random.random() < exploration_rate:
                    ...
                    action = (np.argmax(q1[0]), np.argmax(q2[0]))
                ...
                experience_item_1 = [state, action[0], reward_1, state_next, game_over]
                experience_item_2 = [state, action[1], reward_2, state_next, game_over]
                self.memory_1.remember(*experience_item_1)
                self.memory_2.remember(*experience_item_2)
                state = state_next

                # Sample a random batch from experience.
                batch = self.memory_1.get_batch(
                    model=self.model_1,
            ...
            if checkpoint_freq and (episode % checkpoint_freq) == 0:
                self.model_1.save(f'dqn-1-{episode:08d}.model')
                self.model_2.save(f'dqn-2-{episode:08d}.model')
        ...
        self.model_1.save('dqn-1-final.model')
        self.model_2.save('dqn-2-final.model')
    ...
    def act(self, observation, reward):
        ...
        q1 = self.model_1.predict(state)[0]
        q2 = self.model_2.predict(state)[0]
